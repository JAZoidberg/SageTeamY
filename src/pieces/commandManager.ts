import { Collection, Client, ApplicationCommand, GuildMember, SelectMenuInteraction, ModalSubmitInteraction, TextChannel,
	GuildMemberRoleManager, ButtonInteraction, ModalBuilder, TextInputBuilder, ActionRowBuilder,
	ModalActionRowComponentBuilder, ApplicationCommandType, ApplicationCommandDataResolvable,
	ChannelType, ApplicationCommandPermissionType, TextInputStyle, ChatInputCommandInteraction } from 'discord.js';
...
async function handleDropdown(interaction: SelectMenuInteraction) {
	const courses: Array<Course> = await interaction.client.mongo
		...
		for (const id of removed) {
			...
			if (
				...
				const course = courses.find(
					(c) => c.name === role.name.substring(5)
				);
				const user: SageUser = await interaction.client.mongo
					.collection(DB.USERS)
					.findOne({ discordId: member.id });
				user.courses = user.courses.filter((c) => c !== course.name);
				member.roles.remove(
					course.roles.student,
				...
				interaction.client.mongo
					.collection(DB.USERS)
					.updateOne({ discordId: member.id }, { $set: { ...user } });
				responseContent = `Your enrollments have been updated.`;
			}
		}
...
async function handleModalBuilder(
		...
		case 'verify': {
			...
			if (!entry) {
				interaction.user.send(
					`I could not find that hash in the database. Please try again or contact ${MAINTAINERS}.`
				);
				break;
			}
...
export async function loadCommands(bot: Client): Promise<void> {
	...
	const sageData = (await bot.mongo
		.collection(DB.CLIENT_DATA)
		.findOne({ _id: bot.user.id })) as SageData;
	...
	for (const file of commandFiles) {
		...
		const guildCmd = commands.cache.find(
		...
		if (!guildCmd) {
			...
			numEdited++;
			console.log(
				`a different version of ${command.name} already exists, editing...`
			);
		}

		const oldSettings = oldCommandSettings.find(
			(cmd) => cmd.name === command.name
		);
		let enable: boolean;
		if (oldSettings) {
	...
	console.log(
		`${bot.commands.size} commands loaded (${numNew} new, ${numEdited} edited).`
	);
}
		...
		for (const user of command.permissions) {
			...
			if (user.type === ApplicationCommandPermissionType.Role) {
				...
				if (
					...
					).cache.find((role) => role.id === user.id)
				) {
					success = true;
					break;
				}
