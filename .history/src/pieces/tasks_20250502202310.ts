...
// Trying to fix mongodb error when running jobs command
async function getJobFormData(userID: string, filterBy: string): Promise<[JobData, Interest, JobResult[]]> {
	...
	if (!jobformAnswers || jobformAnswers.length < 2) {
		console.warn(`User ${userID} is missing job form answers in the DB.`);
		// You can either throw an error, return dummy defaults, or send a bot message
		return [
			{
				city: "newark",
				preference: "software",
			...
			{
				interest1: "javascript",
				interest2: "node",
				interest3: "react",
				interest4: "web",
				interest5: "api"
			},
			[] // Or skip fetchJobListings entirely
		];
	}

	const jobData: JobData = {
		city: jobformAnswers[0].answers?.[0] ?? "newark",
		preference: jobformAnswers[0].answers?.[1] ?? "software",
		jobType: jobformAnswers[0].answers?.[2] ?? "full-time",
		distance: jobformAnswers[0].answers?.[3] ?? "10",
		filterBy: filterBy ?? "default"
	};

	const interests: Interest = {
		interest1: jobformAnswers[1].answers?.[0] ?? "javascript",
		interest2: jobformAnswers[1].answers?.[1] ?? "node",
		interest3: jobformAnswers[1].answers?.[2] ?? "react",
		interest4: jobformAnswers[1].answers?.[3] ?? "web",
		interest5: jobformAnswers[1].answers?.[4] ?? "api"
	};
