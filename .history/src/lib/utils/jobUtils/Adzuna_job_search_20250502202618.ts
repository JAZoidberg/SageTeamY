...
export default async function fetchJobListings(jobData: JobData, interests?: Interest): Promise<JobResult[]> {
	//make sure city and jobType don't crash if they're missing
	...
	// make sure interests aren't undefined when building the query string
	let whatInterests = '';
	if (interests) {
	...
	// build the cache key safely using the cleaned-up values
